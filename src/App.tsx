...
const Experience = ({ sceneState, rotationSpeed }: { sceneState: 'CHAOS' | 'FORMED', rotationSpeed: number }) => {
  ...
  useFrame((_state, delta) => {
    if (controlsRef.current) {
      // Tăng tốc độ rotation lên cao hơn (delta * 4.0) để phản hồi rất nhanh
      controlsRef.current.setAzimuthalAngle(controlsRef.current.getAzimuthalAngle() + rotationSpeed * delta * 5.0);
        ...
        if (videoRef.current.videoWidth > 0) {
            ...
            if (results.gestures.length > 0) {
              ...
              if (results.landmarks.length > 0) {
                // Tăng hệ số lên 1.2 để phản hồi rất nhanh
                const speed = (0.5 - results.landmarks[0][0].x) * 1.2;
                // Giảm ngưỡng xuống 0.003 để cực kỳ nhạy
                onMove(Math.abs(speed) > 0.003 ? speed : 0);
              }
